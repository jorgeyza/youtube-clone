...
function EditVideoForm({ videoId, setOpened }: { videoId: string; setOpened: Dispatch<SetStateAction<boolean>> }) {
  ...
  return (
    <form onSubmit={form.onSubmit((values) => mutation.mutate({ videoId, ...values }))}>
      <Stack>
        <TextInput label='Title' required placeholder='My awesome video' {...form.getInputProps('title')} />
  ...
  return (
    <>
      <Modal
        ...
        {mutation.data && <EditVideoForm setOpened={setOpened} videoId={mutation.data.videoId} />}
      </Modal>
